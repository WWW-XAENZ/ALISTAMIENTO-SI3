...
const DB = {
  ...
  async getNoConformes() {
    ...
    const { data, error } = await supabaseClient
      ...
      .order('created_at', { ascending: false });
    if (error) {
      console.error('Error obteniendo no conformes:', error);
      throw error;
    }
    return Array.isArray(data) ? data : [];
  },

  async saveNoConforme(registro) {
    if (!isSupabaseEnabled()) return;
    const { error } = await supabaseClient
      .from('noconformes')
      .insert(registro);
    if (error) {
      console.error('Error guardando no conforme:', error);
      throw error;
    }
  },

  async updateNoConforme(id, updates) {
    if (!isSupabaseEnabled()) return;
    const { error } = await supabaseClient
      .from('noconformes')
      .update(updates)
  ...
  async deleteNoConforme(id) {
    ...
    const { error } = await supabaseClient
      ...
      .eq('id', id);
    if (error) {
      console.error('Error eliminando no conforme:', error);
      throw error;
    }
  },

  onNoConformesChange(callback) {
    if (!isSupabaseEnabled()) return () => {};
    
    const channel = supabaseClient
      .channel('noconformes-changes')
      .on('postgres_changes',
        { event: '*', schema: 'public', table: 'noconformes' },
        callback
      )
      .subscribe((status) => {
        if (status !== 'SUBSCRIBED') {
          console.warn('Estado Realtime noconformes:', status);
        }
      });
