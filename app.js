  ...
  try {
    if (isSupabaseEnabled()) {
      ...
      if (Array.isArray(productos) && productos.length > 0) {
        ...
        const responseLocal = await fetch(JSON_PATH + '?t=' + Date.now());
        const jsonLocal = responseLocal.ok ? await responseLocal.json() : {};
        const locales = Array.isArray(jsonLocal.materiales)
          ? jsonLocal.materiales
          : Array.isArray(jsonLocal) ? jsonLocal : [];
        const porReferencia = new Map(locales.map(p => [
          String(p.referencia || p.producto || '').trim().toLowerCase(),
          p
        ]));
        const combinar = (remoto, local) => {
          const componentes = [...(remoto || []), ...(local || [])];
          const unicos = new Map();
          componentes.forEach(c => {
            const clave = String(c.codigo || `${c.tipo || ''}-${c.descripcion || ''}`).trim();
            if (clave && !unicos.has(clave)) unicos.set(clave, c);
          });
          return Array.from(unicos.values());
        };
        const completos = remotos.map(p => {
          const local = porReferencia.get(String(p.referencia || p.producto || '').trim().toLowerCase());
          if (!local) return p;
          return {
            ...p,
            componentes: combinar(p.componentes, local.componentes),
            componentes_adicionales: combinar(p.componentes_adicionales, local.componentes_adicionales),
            kits: combinar(p.kits, local.kits),
            anti_vibrantes: combinar(p.anti_vibrantes, local.anti_vibrantes),
          };
        });
        const referenciasRemotas = new Set(completos.map(p => String(p.referencia || p.producto || '').trim().toLowerCase()));
        return [...completos, ...locales.filter(p => !referenciasRemotas.has(String(p.referencia || p.producto || '').trim().toLowerCase()))];
      }
    }
...
function obtenerFilasExportacion(registros, productos = []) {
  const filas = [];

  registros.forEach((grupo) => {
    const referencia = grupo.referencia || '';
    const producto = buscarProducto(referencia, productos);
    const componentesProducto = obtenerComponentesProducto(producto);
    const baseComponents = componentesProducto.filter((componente) => esTipoComponente(componente, 'base'));
    const forroComponents = componentesProducto.filter((componente) => esTipoComponente(componente, 'forro'));
    const baseCantidad = normalizarCantidadExportacion(grupo.base);
    const forroCantidad = normalizarCantidadExportacion(grupo.forro);
    let agregoRegistroEstructural = false;

    if (baseCantidad) {
      agregoRegistroEstructural = true;
      const componentesBase = baseComponents.length > 0 ? baseComponents : [{ tipo: 'Base', codigo: '', descripcion: 'BASE' }];
      componentesBase.forEach((componente) => {
        filas.push(crearFilaExportacion({
          tipo: 'BASE',
          codigo: componente.codigo || '',
          nombre: nombreComponente(componente),
          referencia,
    ...
    if (forroCantidad) {
      agregoRegistroEstructural = true;
      const componentesForro = forroComponents.length > 0 ? forroComponents : [{ tipo: 'Forro', codigo: '', descripcion: 'FORRO' }];
      componentesForro.forEach((componente) => {
        filas.push(crearFilaExportacion({
          tipo: 'FORRO',
          codigo: componente.codigo || '',
          nombre: nombreComponente(componente),
          referencia,
          cantidad: forroCantidad,
        }));
      });
    }
    ...
    const items = grupo.items || [];
    items.forEach((item) => {
      const codigo = item.referencia || '';
      const cantidad = normalizarCantidadExportacion(item.componentes);
      ...
      filas.push(crearFilaExportacion({
        tipo: 'COMPONENTE',
        codigo,
        nombre: buscarNombreComponente(codigo, grupo, productos),
        referencia,
        cantidad,
      }));
    });

    if (items.length === 0 && !agregoRegistroEstructural) {
      const cantidad = normalizarCantidadExportacion(grupo.contabilizado || grupo.componentes);
      if (referencia || cantidad) {
        filas.push(crearFilaExportacion({
          tipo: 'REGISTRO',
          codigo: referencia,
          nombre: grupo.descripcion || referencia,
          referencia,
          cantidad,
        }));
      }
...
function crearFilaExportacion({ tipo, codigo, nombre, referencia, cantidad }) {
  return {
    CODIGO: codigo,
    ...
    ORIGEN: 'MP10',
    CENTRO: '4000',
    'ALMACEN TRASPASO': 'PR10',
  };
}

function buscarProducto(referencia, productos) {
  const referenciaBusqueda = String(referencia || '').trim().toUpperCase();
  if (!referenciaBusqueda) return null;

  return productos.find((producto) => String(producto.referencia || producto.producto || '').trim().toUpperCase() === referenciaBusqueda)
    || productos.find((producto) => String(producto.referencia || producto.producto || '').trim().toUpperCase().includes(referenciaBusqueda))
    || null;
}

function obtenerComponentesProducto(producto) {
  if (!producto) return [];

  return [
    ...(Array.isArray(producto.componentes) ? producto.componentes : []),
    ...(Array.isArray(producto.componentes_adicionales) ? producto.componentes_adicionales : []),
    ...(Array.isArray(producto.kits) ? producto.kits : []),
    ...(Array.isArray(producto.anti_vibrantes) ? producto.anti_vibrantes : []),
    ...(producto.subensamble && producto.subensamble.codigo ? [producto.subensamble] : []),
  ];
}

function esTipoComponente(componente, tipo) {
  return String(componente.tipo || '').toLowerCase().includes(tipo);
}

function nombreComponente(componente) {
  return componente.descripcion || componente.tipo || componente.nombre || componente.codigo || '';
}

function buscarNombreComponente(codigo, grupo, productos) {
  const codigoBusqueda = String(codigo || '').trim();
  const componente = obtenerComponentesProducto(buscarProducto(grupo?.referencia, productos))
    .find((elemento) => String(elemento.codigo || '').trim() === codigoBusqueda);

  if (componente) return nombreComponente(componente);

  return grupo?.items?.find((item) => String(item.referencia || '').trim() === codigoBusqueda)?.descripcion || codigoBusqueda;
}

function normalizarCantidadExportacion(cantidad) {
...
function prepararCeldaExcel(valor) {
...
async function exportarRegistrosExcel() {
  try {
    ...
    const filas = obtenerFilasExportacion(registros, productos);
    ...
    const encabezados = ['CODIGO', 'CANTIDAD', 'ORIGEN', 'CENTRO', 'ALMACEN TRASPASO'];
    const lineas = [encabezados.map((encabezado) => prepararCeldaExcel(encabezado)).join(';')];

    filas.forEach((fila) => {
      lineas.push([
        prepararCeldaExcel(fila.CODIGO),
        prepararCeldaExcel(fila.CANTIDAD),
        prepararCeldaExcel(fila.ORIGEN),
        prepararCeldaExcel(fila.CENTRO),
        prepararCeldaExcel(fila['ALMACEN TRASPASO']),
      ].join(';'));
    });
    ...
    enlace.href = url;
    enlace.download = `alistamiento_${getTodayString()}.csv`;
    document.body.appendChild(enlace);
    enlace.click();
    document.body.removeChild(enlace);
    setTimeout(() => URL.revokeObjectURL(url), 1000);
    showToast(`ARCHIVO EXPORTADO: ${filas.length} REGISTROS`, 'success');
  } catch (error) {
    console.error('Error exportando registros:', error);
    showToast('ERROR AL EXPORTAR LOS REGISTROS', 'error');
  }
}
...
function initReferenciaDropdown() {
  ...
  async function actualizarOpciones() {
    ...
    const unicos = new Map();
    filtrados.forEach(r => {
      const clave = String(r.producto || r.referencia || '').trim().toLowerCase();
      const tieneComponentes = (r.componentes || []).length > 0 || (r.componentes_adicionales || []).length > 0;
      const anterior = unicos.get(clave);
      const anteriorTieneComponentes = anterior && ((anterior.componentes || []).length > 0 || (anterior.componentes_adicionales || []).length > 0);
      if (!anterior || (tieneComponentes && !anteriorTieneComponentes)) {
        unicos.set(clave, r);
      }
    });
...
function autocompletarFormulario(producto) {
  ...
  // Base, fomi and forro are entered manually; only the calculated components are automatic.
  const componentesInput = document.getElementById('componentes');
  if (componentesInput) {
    componentesInput.value = '';
  }
...
function multiplicarComponentes() {
  ...
  const lineasAdicionales = adicionales.map(c => {
  ...
  componentesInput.value = lineasAdicionales.join('\n');
}
...
function initMultiplicadorComponentes() {
  ...
  referenciaInput.addEventListener('change', async () => {
    ...
    const unicos = new Map();
    filtrados.forEach(r => {
      const clave = String(r.referencia || r.producto || '').trim().toLowerCase();
      const tieneComponentes = (r.componentes || []).length > 0 || (r.componentes_adicionales || []).length > 0;
      const anterior = unicos.get(clave);
      const anteriorTieneComponentes = anterior && ((anterior.componentes || []).length > 0 || (anterior.componentes_adicionales || []).length > 0);
      if (!anterior || (tieneComponentes && !anteriorTieneComponentes)) {
    ...
    if (productos.length > 0) {
      autocompletarFormulario(productos[0]);
    }
  });
}
...
document.getElementById('btnExportarExcel').addEventListener('click', exportarRegistrosExcel);
...
function initMenu() {
  ...
  if (menuNoConformes) {
    menuNoConformes.addEventListener('click', (e) => {
      e.preventDefault();
      cerrarMenu();
      window.location.href = 'NoConformes.html';
    });
  }
...
async function initTrazabilidadForm() {
